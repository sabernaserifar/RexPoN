...
FixRexPoNBonds::FixRexPoNBonds(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg != 5) error->all(FLERR,"Illegal fix rexpon/bonds command");
  ...
  if (nevery <= 0 )
    error->all(FLERR,"Illegal fix rexpon/bonds command");
  ...
  if (me == 0) {
    ...
    if (fp == NULL) {
      char str[128];
      sprintf(str,"Cannot open fix rexpom/bonds file %s",arg[4]);
      error->one(FLERR,str);
    }
  }

  if (atom->tag_consecutive() == 0)
    error->all(FLERR,"Atom IDs must be consecutive for fix rexpon bonds");
...
void FixRexPoNBonds::setup(int vflag)
{
  end_of_step();
}
...
void FixRexPoNBonds::init()
{
  rexpon = (PairRexPoN *) force->pair_match("rexpon",0);
  if (rexpon == NULL) error->all(FLERR,"Cannot use fix rexpon/bonds without "
                                "pair_style rexpon");
...
void FixRexPoNBonds::end_of_step()
{
...
void FixRexPoNBonds::Output_RexPoN_Bonds(bigint ntimestep, FILE *fp)

{
  ...
  memory->create(buf,nbuf,"rexpon/bonds:buf");
...
void FixRexPoNBonds::FindBond(struct _rexpon_list *lists, int &numbonds)
{
  int *ilist, i, ii, inum;
  int j, pj, nj;
  tagint jtag;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for( pj = Start_Index(i, rexpon->lists); pj < End_Index(i, rexpon->lists); ++pj ) {
      bo_ij = &( rexpon->lists->select.bond_list[pj] );
      j = bo_ij->nbr;
      jtag = tag[j];
...
void FixRexPoNBonds::PassBuffer(double *buf, int &nbuf_local)
{
  int i, j, k, numbonds;
  int nlocal = atom->nlocal;
  ...
  for (i = 0; i < nlocal; i++) {
    buf[j-1] = atom->tag[i];
    buf[j+0] = atom->type[i];
    buf[j+1] = rexpon->workspace->total_bond_order[i];
    buf[j+2] = rexpon->workspace->nlp[i];
    buf[j+3] = atom->q[i];
    buf[j+4] = numneigh[i];
...
void FixRexPoNBonds::RecvBuffer(double *buf, int nbuf, int nbuf_local,
                               int natoms, int maxnum)
{
  ...
  double cutof3 = rexpon->control->bg_cut;
...
int FixRexPoNBonds::nint(const double &r)
{
  int i = 0;
...
void FixRexPoNBonds::allocate()
{
  memory->create(abo,nmax,MAXREXPONBOND,"rexpon/bonds:abo");
  memory->create(neighid,nmax,MAXREXPONBOND,"rexpon/bonds:neighid");
  memory->create(numneigh,nmax,"rexpon/bonds:numneigh");
}

/* ---------------------------------------------------------------------- */

double FixRexPoNBonds::memory_usage()
{
  double bytes;

  bytes = 3.0*nmax*sizeof(double);
  bytes += nmax*sizeof(int);
  bytes += 1.0*nmax*MAXREXPONBOND*sizeof(double);
  bytes += 1.0*nmax*MAXREXPONBOND*sizeof(int);
