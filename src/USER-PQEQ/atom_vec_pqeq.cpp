...
/* ------------------------------------------------------------------
   Contributing author: Saber Naserifar (Caltech)
------------------------------------------------------------------------ */

#include <stdlib.h>
#include "atom_vec_pqeq.h"
#include "atom.h"
#include "comm.h"
#include "domain.h"
#include "modify.h"
...
/* ---------------------------------------------------------------------- */
AtomVecPqeq::AtomVecPqeq(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
  ...
  size_forward = 3;      // 3x
  size_reverse = 3;      // 3f
  size_border = 11;      // 3x,1tag,1type,1mask,1q,3rsx,1molecule
  size_velocity = 3;     // 3v
  size_data_atom = 10;   // ID,molec,type,q,x1,x2,x3,rsx1,rsx2,rsx3
  size_data_vel = 4;     // ID,v1,v2,v3
  xcol_data = 5;         // the column No. of x1
...
void AtomVecPqeq::grow(int n)
{
  ...
  rsx = memory->grow(atom->rsx,nmax,3,"atom:rsx");
...
void AtomVecPqeq::grow_reset()
{
  tag = atom->tag; type = atom->type;
  mask = atom->mask; image = atom->image;
  x = atom->x; v = atom->v; f = atom->f;
  q = atom->q; rsx = atom->rsx; molecule = atom->molecule;
...
void AtomVecPqeq::copy(int i, int j, int delflag)
{
  int k;
...
int AtomVecPqeq::pack_comm(int n, int *list, double *buf,
...
int AtomVecPqeq::pack_comm_vel(int n, int *list, double *buf,
                               int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecPqeq::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecPqeq::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecPqeq::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecPqeq::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecPqeq::pack_border(int n, int *list, double *buf,
                             int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecPqeq::pack_border_vel(int n, int *list, double *buf,
                                 int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecPqeq::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecPqeq::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecPqeq::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecPqeq::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecPqeq::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecPqeq::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecPqeq::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecPqeq::unpack_restart(double *buf)
{
  int k;
...
void AtomVecPqeq::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecPqeq::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  ...
  rsx[nlocal][0] = atof(values[7]);
  rsx[nlocal][1] = atof(values[8]);
  rsx[nlocal][2] = atof(values[9]);
...
------------------------------------------------------------------------- */
int AtomVecPqeq::data_atom_hybrid(int nlocal, char **values)
{
  molecule[nlocal] = ATOTAGINT(values[0]);
  q[nlocal] = atof(values[1]);

  rsx[nlocal][0] = atof(values[2]);
  rsx[nlocal][1] = atof(values[3]);
  rsx[nlocal][2] = atof(values[4]);
...
void AtomVecPqeq::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecPqeq::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecPqeq::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecPqeq::write_data_hybrid(FILE *fp, double *buf)
{
  fprintf(fp," " TAGINT_FORMAT " %-1.16e %-1.16e %-1.16e %-1.16e",(tagint) ubuf(buf[0]).i,buf[1],buf[2],buf[3],buf[4]);
...
bigint AtomVecPqeq::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("rsx")) bytes += memory->usage(rsx,nmax,3);
