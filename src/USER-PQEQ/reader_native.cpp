...
enum{ID,TYPE,X,Y,Z,SX,SY,SZ,VX,VY,VZ,Q,IX,IY,IZ,FX,FY,FZ};
                                 ...
                                 int &xflag, int &yflag, int &zflag)
{
  ...
  for (int i = 0; i < nfield; i++) {
    ...
    else if (fieldtype[i] == SX)
      fieldindex[i] = find_label("sx",nwords,labels);
    else if (fieldtype[i] == SY)
      fieldindex[i] = find_label("sy",nwords,labels);
    else if (fieldtype[i] == SZ)
      fieldindex[i] = find_label("sz",nwords,labels);
