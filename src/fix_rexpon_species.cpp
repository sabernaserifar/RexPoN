...
FixRexPoNSpecies::FixRexPoNSpecies(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 7) error->all(FLERR,"Illegal fix rexpon/species command");
  ...
  if (nevery <= 0 || nrepeat <= 0 || nfreq <= 0)
    error->all(FLERR,"Illegal fix rexpon/species command");
  ...
  if (me == 0 && rene_flag) {
    char str[128];
    sprintf(str,"Resetting reneighboring criteria for fix rexpon/species");
    error->warning(FLERR,str);
  }

  tmparg = NULL;
  memory->create(tmparg,4,4,"rexpon/species:tmparg");
  ...
  if (me == 0) {
    ...
    if (fp == NULL) {
      char str[128];
      sprintf(str,"Cannot open fix rexpon/species file %s",arg[6]);
      error->one(FLERR,str);
    }
  }
  ...
  int ntmp = 1;
  memory->create(x0,ntmp,"rexpon/species:x0");
  memory->create(clusterID,ntmp,"rexpon/species:clusterID");
  ...
  bg_cut = 0.30;
  n = ntypes+1;
  memory->create(BOCut,n,n,"rexpon/species:BOCut");
  for (i = 1; i < n; i ++)
    for (j = 1; j < n; j ++)
      BOCut[i][j] = bg_cut;
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"cutoff") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix rexpon/species command");
      ...
      if (iarg+ntypes+1 > narg) error->all(FLERR,"Illegal fix rexpon/species command");
      ...
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix rexpon/species command");
      posflag = 1;
      ...
      if (strchr(filepos,'*')) {
        ...
        if (me == 0) {
          pos = fopen(filepos, "w");
          if (pos == NULL) error->one(FLERR,"Cannot open fix rexpon/species position file");
        }
        singlepos_opened = 1;
        multipos = 0;
      }
      iarg += 3;
    } else error->all(FLERR,"Illegal fix rexpon/species command");
  }

  if (!eleflag) {
    memory->create(ele,ntypes+1,"rexpon/species:ele");
    ele[0]='C';
    if (ntypes > 1)
      ele[1]='H';
    if (ntypes > 2)
...
FixRexPoNSpecies::~FixRexPoNSpecies()
{
  memory->destroy(ele);
...
void FixRexPoNSpecies::setup(int vflag)
{
  ntotal = static_cast<int> (atom->natoms);
  if (Name == NULL)
    memory->create(Name,ntypes,"rexpon/species:Name");
...
void FixRexPoNSpecies::init()
{
  if (atom->tag_enable == 0)
    error->all(FLERR,"Cannot use fix rexpon/species unless atoms have IDs");

  rexpon = (PairRexPoN *) force->pair_match("rexpon",0);
  if (rexpon == NULL) error->all(FLERR,"Cannot use fix rexpon/species without "
                                "pair_style rexpon");

  rexpon->fixspecies_flag = 1;
  ...
  for (int i = 0; i < modify->nfix; i++)
    if (strcmp(modify->fix[i]->style,"rexpon/species") == 0) count++;
  if (count > 1 && comm->me == 0)
    error->warning(FLERR,"More than one fix rexpon/species");
...
void FixRexPoNSpecies::create_compute()
{
  int narg;
  char **args;
...
void FixRexPoNSpecies::create_fix()
{
  int narg;
  char **args;
...
void FixRexPoNSpecies::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixRexPoNSpecies::post_integrate()
{
  Output_RexPoN_Bonds(update->ntimestep,fp);
  if (me == 0) fflush(fp);
}

/* ---------------------------------------------------------------------- */

void FixRexPoNSpecies::Output_RexPoN_Bonds(bigint ntimestep, FILE *fp)

{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->destroy(x0);
    memory->destroy(clusterID);
    memory->create(x0,nmax,"rexpon/species:x0");
    memory->create(clusterID,nmax,"rexpon/species:clusterID");
    vector_atom = clusterID;
  }
...
AtomCoord FixRexPoNSpecies::chAnchor(AtomCoord in1, AtomCoord in2)
{
  if (in1.x < in2.x)
    return in1;
  else if (in1.x == in2.x) {
...
void FixRexPoNSpecies::FindMolecule ()
{
  ...
  inum = rexpon->list->inum;
  ilist = rexpon->list->ilist;
...
void FixRexPoNSpecies::SortMolecule(int &Nmole)
{
  ...
  if (idlo == ntotal)
    if (me == 0)
      error->warning(FLERR,"Atom with cluster ID = maxmol "
                     "included in fix rexpon/species group");

  int nlen = idhi - idlo + 1;
  memory->create(molmap,nlen,"rexpon/species:molmap");
  ...
  int *molmapall;
  memory->create(molmapall,nlen,"rexpon/species:molmapall");
...
void FixRexPoNSpecies::FindSpecies(int Nmole, int &Nspec)
{
  ...
  memory->destroy(MolName);
  MolName = NULL;
  memory->create(MolName,Nmole*(ntypes+1),"rexpon/species:MolName");

  memory->destroy(NMol);
  NMol = NULL;
  memory->create(NMol,Nmole,"rexpon/species:NMol");
  for (m = 0; m < Nmole; m ++)
    NMol[m] = 1;

  memory->create(Nameall,ntypes,"rexpon/species:Nameall");
  memory->create(NMolall,Nmole,"rexpon/species:NMolall");
  ...
  memory->destroy(nd);
  nd = NULL;
  memory->create(nd,Nspec,"rexpon/species:nd");

  memory->destroy(MolType);
  MolType = NULL;
  memory->create(MolType,Nspec*(ntypes+2),"rexpon/species:MolType");
}

/* ---------------------------------------------------------------------- */

int FixRexPoNSpecies::CheckExistence(int id, int ntypes)
{
  int i, j, molid, flag;
...
void FixRexPoNSpecies::WriteFormulas(int Nmole, int Nspec)
{
  int i, j, itemp;
...
void FixRexPoNSpecies::WritePos(int Nmole, int Nspec)
{
  int i, itype, cid;
...
double FixRexPoNSpecies::compute_vector(int n)
{
  if (n == 0)
    return vector_nmole;
  if (n == 1)
...
int FixRexPoNSpecies::nint(const double &r)
{
  int i = 0;
...
int FixRexPoNSpecies::pack_forward_comm(int n, int *list, double *buf,
...
void FixRexPoNSpecies::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double FixRexPoNSpecies::memory_usage()
{
  double bytes;
