...
void Reset_Atoms( rexpon_system* system, control_params *control )
{
  int i;
  rexpon_atom *atom;
  ...
  if( control->hbond_cut > 0 )
    for( i = 0; i < system->n; ++i ) {
      ...
      if( system->rexpon_param.sbp[ atom->type ].p_hbond == 1 )
...
void Reset_Timing( rexpon_timing *rt )
{
  rt->total = Get_Time();
  rt->comm = 0;
  rt->nbrs = 0;
...
void Reset_Workspace( rexpon_system *system, storage *workspace )
{
...
void Reset_Neighbor_Lists( rexpon_system *system, control_params *control,
...
void Reset( rexpon_system *system, control_params *control, simulation_data *data,
            storage *workspace, rexpon_list **lists, MPI_Comm comm )
{
