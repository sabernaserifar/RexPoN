...
void Atom_Energy( rexpon_system *system, control_params *control,
                  ...
                  output_controls *out_control )
{
  ...
  p_lp3 = system->rexpon_param.gp.l[5];
  p_ovun3 = system->rexpon_param.gp.l[32];
  p_ovun4 = system->rexpon_param.gp.l[31];
  p_ovun6 = system->rexpon_param.gp.l[6];
  p_ovun7 = system->rexpon_param.gp.l[8];
  p_ovun8 = system->rexpon_param.gp.l[9];
  ...
  for( i = 0; i < system->n; ++i ) {
    ...
    /* correction for C2 */
    if( p_lp3 > 0.001 && !strcmp(system->rexpon_param.sbp[type_i].name, "C") )
      for( pj = Start_Index(i, bonds); pj < End_Index(i, bonds); ++pj ) {
        j = bonds->select.bond_list[pj].nbr;
        type_j = system->my_atoms[j].type;
        if (type_j < 0) continue;

        if( !strcmp( system->rexpon_param.sbp[type_j].name, "C" ) ) {
