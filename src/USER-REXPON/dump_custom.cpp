...
int DumpCustom::count()
{
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      } else if (thresh_array[ithresh] == SX) {
        ptr = &atom->rsx[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == SY) {
        ptr = &atom->rsx[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == SZ) {
        ptr = &atom->rsx[0][2];
        nstride = 3;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    } else if (strcmp(arg[iarg],"sx") == 0) {
      pack_choice[i] = &DumpCustom::pack_sx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"sy") == 0) {
      pack_choice[i] = &DumpCustom::pack_sy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"sz") == 0) {
      pack_choice[i] = &DumpCustom::pack_sz;
      vtype[i] = DOUBLE;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"sx") == 0) thresh_array[nthresh] = SX;
    else if (strcmp(arg[1],"sy") == 0) thresh_array[nthresh] = SY;
    else if (strcmp(arg[1],"sz") == 0) thresh_array[nthresh] = SZ;
