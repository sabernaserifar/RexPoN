...
int PreAllocate_Space( rexpon_system *system, control_params *control,
                       storage *workspace, MPI_Comm comm )
{
  ...
  system->my_atoms = (rexpon_atom*)
    scalloc( system->total_cap, sizeof(rexpon_atom), "my_atoms", comm );
...
int Allocate_System( rexpon_system *system, int local_cap, int total_cap,
                     char *msg )
{
  system->my_atoms = (rexpon_atom*)
    realloc( system->my_atoms, total_cap*sizeof(rexpon_atom) );

  return SUCCESS;
}


void DeAllocate_System( rexpon_system *system )
{
  int i, j, k;
  int ntypes;
  rexpon_interaction *ff_params;

  // dealloocate the atom list
  sfree( system->my_atoms, "system->my_atoms" );

  // deallocate the ffield parameters storage
  ff_params = &(system->rexpon_param);
...
int Allocate_Workspace( rexpon_system *system, control_params *control,
...
static void Reallocate_Neighbor_List( rexpon_list *far_nbrs, int n,
...
static int Reallocate_HBonds_List( rexpon_system *system, rexpon_list *hbonds,
                                   MPI_Comm comm )
{
  int i, total_hbonds;
...
static int Reallocate_Bonds_List( rexpon_system *system, rexpon_list *bonds,
...
void ReAllocate( rexpon_system *system, control_params *control,
