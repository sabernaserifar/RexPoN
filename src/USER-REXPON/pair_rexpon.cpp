...
#include "rexpon_allocate.h"
#include "rexpon_control.h"
#include "rexpon_ffield.h"
#include "rexpon_forces.h"
#include "rexpon_init_md.h"
#include "rexpon_io_tools.h"
#include "rexpon_list.h"
#include "rexpon_lookup.h"
#include "rexpon_reset_tools.h"
#include "rexpon_traj.h"
#include "rexpon_vector.h"
#include "fix_rexpon_bonds.h"

using namespace LAMMPS_NS;

static const char cite_pair_rexpon[] =
  "pair rexpon command:\n\n"
  "@Article{Naserifar,\n"
  " author = {S. Naserifar, W. A. Goddard III},\n"
  " title = {},\n"
  " journal = {},\n"
  " year =    2018,\n"
  " volume =  ,\n"
...
PairRexPoN::PairRexPoN(LAMMPS *lmp) : Pair(lmp)
{
  if (lmp->citeme) lmp->citeme->add(cite_pair_rexpon);
  ...
  system = (rexpon_system *)
    memory->smalloc(sizeof(rexpon_system),"rexpon:system");
  control = (control_params *)
    memory->smalloc(sizeof(control_params),"rexpon:control");
  data = (simulation_data *)
    memory->smalloc(sizeof(simulation_data),"rexpon:data");
  workspace = (storage *)
    memory->smalloc(sizeof(storage),"rexpon:storage");
  lists = (rexpon_list *)
    memory->smalloc(LIST_N * sizeof(rexpon_list),"rexpon:lists");
  memset(lists,0,LIST_N * sizeof(rexpon_list));
  out_control = (output_controls *)
    memory->smalloc(sizeof(output_controls),"rexpon:out_control");
  mpi_data = (mpi_datatypes *)
    memory->smalloc(sizeof(mpi_datatypes),"rexpon:mpi");
...
PairRexPoN::~PairRexPoN()
{
  if (copymode) return;

  if (fix_rexpon) modify->delete_fix("REXPON");

  if (setup_flag) {
    Close_Output_Files( system, control, out_control, mpi_data );

    // deallocate rexpon data-structures
...
void PairRexPoN::settings(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"checkqeq") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal pair_style rexpon command");
      if (strcmp(arg[iarg+1],"yes") == 0) qeqflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) qeqflag = 0;
      else error->all(FLERR,"Illegal pair_style rexpon command");
      iarg += 2;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"coulomb_off") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal pair_style rexpon command");
      if (strcmp(arg[iarg+1],"yes") == 0) control-> coulomb_off_flag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) control-> coulomb_off_flag = 0;
      else error->all(FLERR,"Illegal pair_style rexpon command");
      iarg += 2;
	...
	error->all(FLERR,"Illegal pair_style rexpon safezone command");
      ...
      if (system->mincap < 0)
	error->all(FLERR,"Illegal pair_style rexpon mincap command");
      iarg += 2;
    } else error->all(FLERR,"Illegal pair_style rexpon command");
  }
...
void PairRexPoN::coeff( int nargs, char **args )
{
  ...
  if (fp != NULL)
    Read_Force_Field(fp, &(system->rexpon_param), control);
  else {
      char str[128];
      sprintf(str,"Cannot open rexponFF potential file %s",file);
      error->all(FLERR,str);
  }
  ...
  int itmp = 0;
  int nrexpon_types = system->rexpon_param.num_atom_types;
  ...
  for (int i = 3; i < nargs; i++)
    for (int j = 0; j < nrexpon_types; j++)
      if (strcasecmp(args[i],system->rexpon_param.sbp[j].name) == 0) {
        map[i-2] = j;
	itmp ++;
      }
  ...
  // error check
  if (itmp != n)
    error->all(FLERR,"Non-existent rexponFF type");
...
void PairRexPoN::init_style( )
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style rexpon requires atom attribute q");
  ...
  if (iqeq == modify->nfix && qeqflag == 1)
    error->all(FLERR,"Pair rexpon requires use of fix qeq/reax");
  ...
  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style rexpon requires atom IDs");
  if (force->newton_pair == 0)
    error->all(FLERR,"Pair style rexpon requires newton pair on");
  ...
  if (fix_rexpon == NULL) {
    ...
    fixarg[0] = (char *) "REXPON";
    fixarg[1] = (char *) "all";
    fixarg[2] = (char *) "REXPON";
    modify->add_fix(3,fixarg);
    delete [] fixarg;
    fix_rexpon = (FixRexPoN *) modify->fix[modify->nfix-1];
  }
}

/* ---------------------------------------------------------------------- */

void PairRexPoN::setup( )
{
  int oldN;
  ...
  if (setup_flag == 0) {
    ...
    int *num_bonds = fix_rexpon->num_bonds;
    int *num_hbonds = fix_rexpon->num_hbonds;
    ...
    int num_nbrs = estimate_rexpon_lists();
    if(!Make_List(system->total_cap, num_nbrs, TYP_FAR_NEIGHBOR,
                  lists+FAR_NBRS, world))
      error->all(FLERR,"Pair rexpon problem in far neighbor list");
...
double PairRexPoN::init_one(int i, int j)
{
...
void PairRexPoN::compute(int eflag, int vflag)
{
  double evdwl,ecoul;
  ...
  if (neighbor->ago == 0) comm->forward_comm_fix(fix_rexpon);
  int *num_bonds = fix_rexpon->num_bonds;
  int *num_hbonds = fix_rexpon->num_hbonds;
  ...
  workspace->realloc.num_far = write_rexpon_lists();
  // timing for filling in the rexpon lists
  if( comm->me == 0 ) {
    t_end = MPI_Wtime();
...
// Set internal timestep counter to that of LAMMPS
  ...
  // populate tmpid and tmpbo arrays for fix rexpon/c/species
  int i, j;
...
void PairRexPoN::write_rexpon_atoms()
{
  int *num_bonds = fix_rexpon->num_bonds;
  int *num_hbonds = fix_rexpon->num_hbonds;
  ...
  for( int i = 0; i < system->N; ++i ){
    ...
    system->my_atoms[i].molec_id = atom->molecule[i];
...
void PairRexPoN::get_distance( rvec xj, rvec xi, double *d_sqr, rvec *dvec )
{
  (*dvec)[0] = xj[0] - xi[0];
  (*dvec)[1] = xj[1] - xi[1];
  (*dvec)[2] = xj[2] - xi[2];
...
void PairRexPoN::set_far_nbr( far_neighbor_data *fdest,
...
int PairRexPoN::estimate_rexpon_lists()
{
  int itr_i, itr_j, i, j;
...
int PairRexPoN::write_rexpon_lists()
{
  int itr_i, itr_j, i, j;
  int num_nbrs;
...
void PairRexPoN::read_rexpon_forces(int vflag)
{
  for( int i = 0; i < system->N; ++i ) {
...
void *PairRexPoN::extract(const char *str, int &dim)
{
  dim = 1;
  if (strcmp(str,"chi") == 0 && chi) {
    for (int i = 1; i <= atom->ntypes; i++)
      if (map[i] >= 0) chi[i] = system->rexpon_param.sbp[map[i]].chi;
      else chi[i] = 0.0;
    return (void *) chi;
  }
  if (strcmp(str,"eta") == 0 && eta) {
    for (int i = 1; i <= atom->ntypes; i++)
      if (map[i] >= 0) eta[i] = system->rexpon_param.sbp[map[i]].eta;
      else eta[i] = 0.0;
    return (void *) eta;
  }
  if (strcmp(str,"gamma") == 0 && gamma) {
    for (int i = 1; i <= atom->ntypes; i++)
      if (map[i] >= 0) gamma[i] = system->rexpon_param.sbp[map[i]].gamma;
      else gamma[i] = 0.0;
    return (void *) gamma;
  }
  return NULL;
...
double PairRexPoN::memory_usage()
{
  double bytes = 0.0;
  ...
  // From rexponc_allocate: BO
  bytes += 1.0 * system->total_cap * sizeof(rexpon_atom);
...
void PairRexPoN::FindBond()
{
  ...
  for (i = 0; i < system->n; i++) {
	...
	tmpbo[i][nj] = bo_tmp;
	nj ++;
	if (nj > MAXSPECBOND) error->all(FLERR,"Increase MAXSPECBOND in rexpon_defs.h");
      }
    }
  }
}
