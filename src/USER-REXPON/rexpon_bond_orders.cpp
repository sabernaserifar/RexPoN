...
void Add_dBond_to_Forces_NPT( int i, int pj, simulation_data *data,
                              storage *workspace, rexpon_list **lists )
{
...
void Add_dBond_to_Forces( rexpon_system *system, int i, int pj,
                          storage *workspace, rexpon_list **lists )
{
  rexpon_list *bonds = (*lists) + BONDS;
  bond_data *nbr_j, *nbr_k;
...
int BOp( storage *workspace, rexpon_list *bonds, double bo_cut,
  ...
  if( sbp_i->r_s > 0.0 && sbp_j->r_s > 0.0 ) {
    ...
    C12 = twbp->p_bo1 * ( nbr_pj->d - twbp->p_bo2 ) / ( twbp->r_s - twbp->p_bo2 );
    BO_s = 1.10 * (1.0 +  0.00000) / (1.0 + exp( -C12 ) );
    //C12 = -twbp->p_bo1 * pow( nbr_pj->d / twbp->r_s, twbp->p_bo2 );
    //BO_s = (1.1) * exp( C12 );
    f_C12 = 1.0;
  }
  ...
  if( BO >= bo_cut ) {
    ...
    Cln_BOp_s = f_C12 * twbp->p_bo1/(twbp->r_s-twbp->p_bo2) * exp(-C12)/(1.0+exp(-C12)) / nbr_pj->d;
    //Cln_BOp_s = - twbp->p_bo2 * C12 / r2;
...
void BO( rexpon_system *system, control_params *control, simulation_data *data,
         storage *workspace, rexpon_list **lists, output_controls *out_control )
{
  ...
  p_boc1 = system->rexpon_param.gp.l[0];
  p_boc2 = system->rexpon_param.gp.l[1];
 ...
 /* Calculate Deltaprime, Deltaprime_boc values */
    ...
    for( pj = start_i; pj < end_i; ++pj ) {
      ...
      if( i < j || workspace->bond_mark[j] > 3 ) {
        ...
        if( twbp->ovc < 0.001 && twbp->v13cor < 0.001 ) {
          ...
          val_j = system->rexpon_param.sbp[type_j].valency;
          ...
          if( twbp->v13cor >= 0.001 ) {
            ...
            // no correction if in same mole and set to zero if from diff molec
            if (system->my_atoms[i].molec_id == system->my_atoms[j].molec_id ) {
              if (type_i == type_j) {
               f4 = f5 = f4f5 = 0.0;
               Cf45_ij = Cf45_ji = 0.0;
             } else {
  ...
  p_lp1 = system->rexpon_param.gp.l[15];
