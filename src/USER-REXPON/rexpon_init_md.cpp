...
#include "rexpon_system_props.h"
#include "rexpon_tool_box.h"
#include "rexpon_vector.h"

int Init_System( rexpon_system *system, control_params *control, char *msg )
{
  int i;
  rexpon_atom *atom;
  ...
  if( control->hbond_cut > 0 )
    for( i = 0; i < system->n; ++i ) {
      ...
      if (system->rexpon_param.sbp[ atom->type ].p_hbond == 1 && atom->type >= 0)
...
int Init_Simulation_Data( rexpon_system *system, control_params *control,
...
void Init_Taper( control_params *control,  storage *workspace, MPI_Comm comm )
{
  ...
  workspace->TapOO[7] =  20.0 / d7;
  workspace->TapOO[6] = -70.0 * (swa + swb) / d7;
  workspace->TapOO[5] =  84.0 * (swa2 + 3.0*swa*swb + swb2) / d7;
  workspace->TapOO[4] = -35.0 * (swa3 + 9.0*swa2*swb + 9.0*swa*swb2 + swb3 ) / d7;
  workspace->TapOO[3] = 140.0 * (swa3*swb + 3.0*swa2*swb2 + swa*swb3 ) / d7;
  workspace->TapOO[2] =-210.0 * (swa3*swb2 + swa2*swb3) / d7;
  workspace->TapOO[1] = 140.0 * swa3 * swb3 / d7;
  workspace->TapOO[0] = (-35.0*swa3*swb2*swb2 + 21.0*swa2*swb3*swb2 +
                     7.0*swa*swb3*swb3 + swb3*swb3*swb ) / d7;

  swa = 0.0;
  swb = control->hbond_cut-0.96;
  ...
  workspace->TapHH[7] =  20.0 / d7;
  workspace->TapHH[6] = -70.0 * (swa + swb) / d7;
  workspace->TapHH[5] =  84.0 * (swa2 + 3.0*swa*swb + swb2) / d7;
  workspace->TapHH[4] = -35.0 * (swa3 + 9.0*swa2*swb + 9.0*swa*swb2 + swb3 ) / d7;
  workspace->TapHH[3] = 140.0 * (swa3*swb + 3.0*swa2*swb2 + swa*swb3 ) / d7;
  workspace->TapHH[2] =-210.0 * (swa3*swb2 + swa2*swb3) / d7;
  workspace->TapHH[1] = 140.0 * swa3 * swb3 / d7;
  workspace->TapHH[0] = (-35.0*swa3*swb2*swb2 + 21.0*swa2*swb3*swb2 +
                     7.0*swa*swb3*swb3 + swb3*swb3*swb ) / d7;
...
int Init_Workspace( rexpon_system *system, control_params *control,
...
int Init_MPI_Datatypes( rexpon_system *system, storage *workspace,
...
int  Init_Lists( rexpon_system *system, control_params *control,
...
void Initialize( rexpon_system *system, control_params *control,
