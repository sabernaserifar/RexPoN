...
#include "rexpon_multi_body.h"
#include "rexpon_nonbonded.h"
#include "rexpon_tool_box.h"
#include "rexpon_torsion_angles.h"
#include "rexpon_valence_angles.h"
#include "rexpon_vector.h"

interaction_function Interaction_Functions[NUM_INTRS];

void Dummy_Interaction( rexpon_system *system, control_params *control,
...
void Compute_Bonded_Forces( rexpon_system *system, control_params *control,
                            simulation_data *data, storage *workspace,
                            rexpon_list **lists, output_controls *out_control,
                            MPI_Comm comm )
{
...
void Compute_NonBonded_Forces( rexpon_system *system, control_params *control,
                               simulation_data *data, storage *workspace,
                               rexpon_list **lists, output_controls *out_control,
                               MPI_Comm comm )
{
...
void Compute_Total_Force( rexpon_system *system, control_params *control,
                          simulation_data *data, storage *workspace,
                          rexpon_list **lists, mpi_datatypes *mpi_data )
{
  int i, pj;
  rexpon_list *bonds = (*lists) + BONDS;
...
void Validate_Lists( rexpon_system *system, storage *workspace, rexpon_list **lists,
...
void Init_Forces_noQEq( rexpon_system *system, control_params *control,
                        simulation_data *data, storage *workspace,
                        rexpon_list **lists, output_controls *out_control,
                        MPI_Comm comm ) {
  int i, j, pj;
  ...
  double cutoff;
  rexpon_list *far_nbrs, *bonds, *hbonds;
  single_body_parameters *sbp_i, *sbp_j;
  two_body_parameters *twbp;
  far_neighbor_data *nbr_pj;
  rexpon_atom *atom_i, *atom_j;
...
void Estimate_Storages( rexpon_system *system, control_params *control,
                        rexpon_list **lists, int *Htop, int *hb_top,
    ...
    for( pj = start_i; pj < end_i; ++pj ) {
      ...
      if(nbr_pj->d <= cutoff) {
        ...
        if( local ) {
          ...
          /* hydrogen bond lists */
          if( control->hbond_cut > 0.1 && (ihb==1)  &&
              nbr_pj->d <= control->hbond_cut ) {
            jhb = sbp_j->p_hbond;
            if( jhb == 1 ) {
              ++hb_top[i];
        ...
        if( nbr_pj->d <= control->bond_cut ) {
          if( sbp_i->r_s > 0.0 && sbp_j->r_s > 0.0) {
            ...
            C12 = twbp->p_bo1 * ( r_ij - twbp->p_bo2 ) / ( twbp->r_s - twbp->p_bo2 );
            BO_s = 1.0 * (1.0 + 0.0) / (1.0 + exp( -C12 ) );
          }
          else BO_s = C12 = 0.0;
...
void Compute_Forces( rexpon_system *system, control_params *control,
                     simulation_data *data, storage *workspace,
                     rexpon_list **lists, output_controls *out_control,
