...
void Valence_Angles( rexpon_system *system, control_params *control,
                     ...
                     rexpon_list **lists, output_controls *out_control )
{
  ...
  double cos2theta,costheta,cangle,theta0;  //VaPoX
  ...
  rexpon_list *thb_intrs =  (*lists) + THREE_BODIES;

  /* global parameters used in these calculations */
  p_val6 = system->rexpon_param.gp.l[14];
  p_val8 = system->rexpon_param.gp.l[33];
  p_val9 = system->rexpon_param.gp.l[16];
  p_val10 = system->rexpon_param.gp.l[17];
  num_thb_intrs = 0;
  ...
  for( j = 0; j < system->N; ++j ) {         // Ray: the first one with system->N
    ...
    p_val3 = system->rexpon_param.sbp[ type_j ].p_val3;
    p_val5 = system->rexpon_param.sbp[ type_j ].p_val5;
      ...
      if( BOA_ij/*bo_ij->BO*/ > 0.0 &&
        ...
        for( pk = pi+1; pk < end_j; ++pk ) {
          ...
          if( (j < system->n) && (BOA_jk > 0.0) &&
            ...
            thbh = &( system->rexpon_param.thbp[ type_i ][ type_j ][ type_k ] );
            ...
            for( cnt = 0; cnt < thbh->cnt; ++cnt ) {
              if( fabs(thbh->prm[cnt].p_val1) > 0.001 ) {
                ...
                // VaPoX
                theta0 = DEG2RAD(theta_00);
                cangle = 2.0 * p_val1 / pow( sin(theta0),2.0 );
                costheta = cos(theta) - cos(theta0);
                cos2theta = pow( costheta,2.0 );
                // End VaPoX
                ...
                CEval1 = cangle * Cf7ij * f7_jk * cos2theta;
                CEval2 = cangle * Cf7jk * f7_ij * cos2theta;
                CEval3 = 0.0;
                CEval4 = 2.0 * cangle * f7_ij * f7_jk * costheta;
                ...
                /* PENALTY ENERGY */
                p_pen1 = thbp->p_pen1;
                p_pen2 = system->rexpon_param.gp.l[19];
                p_pen3 = system->rexpon_param.gp.l[20];
                p_pen4 = system->rexpon_param.gp.l[21];
                ...
                /* COALITION ENERGY */
                p_coa1 = thbp->p_coa1;
                p_coa2 = system->rexpon_param.gp.l[2];
                p_coa3 = system->rexpon_param.gp.l[38];
                p_coa4 = system->rexpon_param.gp.l[30];
